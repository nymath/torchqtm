...
from torchqtm.assets import Asset, Future
from torchqtm.finance.orders.order import Order
from torchqtm.constants import ROOT_SYMBOL_TO_ETA
...
def fill_price_worse_than_limit_price(fill_price: float, order: Order) -> bool:
    """Checks whether the fill price is worse than the order's limit price

    Note
    ----
    In fact, the order is going to be filled iff the current price is worse than the order's limit price
    """
...
class SlippageModel(object, metaclass=ABCMeta):
    ...
    volume_for_bar : int
        ...
        multiple open get_orders for a single asset.
    """
    allowed_asset_types = (ASSET_TYPE.Equity, ASSET_TYPE.Future)

    # class OrderTransactionIterator:
    #     def __init__(self, slippage_model, data, asset, orders_for_asset):
    #         self.slippage_model = slippage_model
    #         self.data = data
    #         self.asset = asset
    #         self.orders_for_asset = orders_for_asset
    #         self.index = 0
    #
    #     def __iter__(self):
    #         return self
    #
    #     def __next__(self):
    #         while self.index < len(self.orders_for_asset):
    #             order = self.orders_for_asset[self.index]
    #             self.index += 1
    #
    #             if order.open_amount == 0:
    #                 continue
    #
    #             order.check_trigger(self.data.current(self.asset, "close"), self.data.current_dt)
    #             if not order.triggered:
    #                 continue
    #
    #             txn = None
    #             try:
    #                 execution_price, execution_volume = self.slippage_model.process_order(self.data, order)
    #                 if execution_price is not None:
    #                     txn = Transaction(
    #                         asset=order.asset,
    #                         amount=execution_volume,
    #                         dt=self.data.current_dt,
    #                         price=execution_price,
    #                         order_id=order.id
    #                     )
    #             except LiquidityExceeded:
    #                 print("Liquidity Exceeded")
    #                 break
    #
    #             if txn:
    #                 self.slippage_model._volume_for_bar += abs(txn.amount)
    #                 return order, txn
    #
    #         raise StopIteration

    def __init__(self):
        self._volume_for_bar: int = 0
    ...
    def process_order(self, data: DataPortal, order: Order) -> Tuple[float, int]:
        ...
        """
        raise NotImplementedError(f"{self.__class__.__name__}.process_order")

    def simulate(self,
                 data: DataPortal,
                 asset: Asset,
                 orders_for_asset: Iterable[Order]
                 ) -> Optional[List[Tuple[Order, Transaction]]]:
        rlts = []
...
class NoSlippage(SlippageModel):
    """A slippage model where all get_orders fill immediately and completely at the current close price

    """

    def process_order(self, data: DataPortal, order: Order) -> Tuple[float, float]:
        return data.current(order.asset, "close"), order.amount


class EquitySlippageModel(SlippageModel, metaclass=ABCMeta):
    """Base class for slippage models which only support equities."""

    allowed_asset_types = (ASSET_TYPE.Equity,)


class FutureSlippageModel(SlippageModel, metaclass=ABCMeta):
    """Base class for slippage models which only support futures."""

    allowed_asset_types = (ASSET_TYPE.Future,)
...
class VolumeShareSlippage(SlippageModel):
    ...
    def __init__(
            self,
            volume_limit: float = DEFAULT_EQUITY_VOLUME_SLIPPAGE_BAR_LIMIT,
...
""".strip().format(
    ...
    def process_order(self, data: DataPortal, order: Order):
        volume = data.current(order.asset, "volume")

        max_volume = self.volume_limit * volume

        # price impact accounts for the total volume of get_transactions
        # created against the current minute bar
        remaining_volume = max_volume - self.volume_for_bar
        if remaining_volume < 1:
            # we can't fill any more get_transactions
...
class FixedSlippage(SlippageModel):
    ...
    def process_order(self, data: DataPortal, order):
        price = data.current(order.asset, "close")
